...
| T-014 security/destructive-ops/docs sweep | CLOSED | Secret hygiene -- extended to `.saipen/kitchen/`/`.saitranslate/kitchen/` (`grep -n "MUST NOT write secrets" saipen/RFC.md`). Destructive-ops rule -- added to RFC § 1.1 (`grep -n "MUST NOT perform a destructive operation" saipen/RFC.md`): force-push, branch deletion, history rewrite, schema drop, mass file deletion, user data deletion, irreversible migration, all requiring confirmation unless the active ticket pre-authorizes AND it's reversible. Docs sweep -- actually done, not assumed clean: `GUIDE.md`/`GUIDE_EN.md`/`GUIDE_RU.md` command tables were missing `saipen translate`, `saipen ship`, and `saipen validate` -- 3 of 9 real commands absent from every one of the three guide variants, same gap in all three (`grep -n "saipen translate\|saipen ship\|saipen validate" GUIDE*.md` -> zero hits before this ticket, present after). `README.md`'s version badge had drifted stale to `v7.6.0` again (real: `v7.15.0`) -- this is the second time this exact drift has happened; `SPEC.md`'s `requires:` example was missing `shell` relative to RFC's own example. |
