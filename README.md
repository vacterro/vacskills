# vacskill вЂ” One Skill. Any Agent. Zero Amnesia.

**v3.1.1** В· [Changelog](CHANGELOG.md) В· plain markdown В· zero dependencies В· MIT
...
with that. Open ANY other agent, say `VACSKILL SET` вЂ” it reads the
...
Models come and go вЂ” Claude today, GPT tomorrow, some local Llama on a
toaster next year. The **notebook stays**. Memory owns the project; the
model is just a temporary worker with a shovel.

```
vacskill/                   в†ђ this repo, clone anywhere
  vacskill/SKILL.md         cold protocol вЂ” how robots work, no feelings
  vacskill/STYLE.md         the voices вЂ” kept OUT of the protocol's way
  vacskill/UI.md            the look (Win95 dark gold) вЂ” loaded only for UI
  inject.ps1 / inject.sh    one-shot installer

your-project/.vacskill/     в†ђ the notebook, robots build it themselves
  STATE.md                  where we are + the EXACT next command
  BOARD.md                  tickets with needs: dependencies вЂ” the boss
  LOG.md                    diary: every run, every decision, one line each
  KNOWLEDGE/                what's TRUE: architecture, conventions,
                            decisions, traps вЂ” outlives every damn model
```

Diary says what HAPPENED. Knowledge says what's TRUE. Mixing those two is
how projects end up with a 1800-line log nobody reads вЂ” we don't do that
here.

## Install вЂ” three commands, no crying
...
The injector sniffs out every agent on your machine вЂ” Claude Code,
OpenCode, Codex, Gemini/Antigravity, Aider, generic `~/.agents` readers вЂ”
...
| `vacskill` / `VACSKILL SET` | continues from notebook; empty board в†’ hunts bugs itself |
| `vacskill fix <thing>` | reproduce в†’ root cause в†’ regression test. No guessing |
| `vacskill status` | reads notebook + quick numbers, touches nothing |
| `vacskill stop` | proper goodbye note, safe to walk away |
| `vacskill ship` | inspection в†’ 100% green в†’ YOUR GitHub, versioned + changelog |

**Every ticket walks the same road:** SCOUT (read how the repo already
does it вЂ” inventing a parallel architecture is how agents ruin projects) в†’
BUILD (smallest safe change, no stubs, no "TODO later" lies) в†’ VERIFY
(run it for real; ticket closes with honest confidence: high/med/low).
REVIEW guards the whole diff before ship: correctness в†’ security в†’
...
first" вЂ” no. T-001. Dig. Several agents at once? Each claims a `[P]`
ticket, DAG keeps them out of each other's pants, a join ticket re-checks
the merge.

**Death is budgeted.** Checkpoints after EVERY ticket вЂ” a dying agent gets
...
deleted at stop. Orphan files get deleted only when PROVEN unreferenced вЂ”
...
session вЂ” because "fixed the damn timeout AGAIN" is remembered a month
...
- Every loop has a hard cap: 3 dead hypotheses or 2 failed fixes в†’
  ticket BLOCKED with facts, next ticket. No robot spinning in circles
  burning your money at 3am.
- Publish is opt-in only вЂ” your projects never land on GitHub uninvited.
- Two agents, one project в†’ takeover guard asks before grabbing the wheel.
- Versions grow slow and honest: `3.1.0 в†’ 3.1.0a` micro, `3.2.1` little,
  `3.2.0` feature. No jumping to v9 because the README got a new emoji.
- Every shipped state is tagged and recoverable forever вЂ” `git show
...
the injector вЂ” it strips the old `VAC` blocks, drops the old skill dirs and
...
Improvements come from real usage pain only вЂ” speculative features are fat,
not muscle.
