...
`saipen GOAL <text>` is an explicit, session-scoped opt-in that (a) sets a new high-level objective, superseding whatever was queued, and (b) runs that objective to completion with minimal interruption. It is NOT the default — `saipen <text>` and bare `saipen continue` retain today's behavior.
