...
4. **Transition**: If fixed or passed, transition `STATE.phase` to `SCOUT` (if workable tickets exist), `PLAN` (if unrefined tasks exist), or `DONE` (if the board is empty, which will trigger Autonomous Evolution per RFC § 2.1).
